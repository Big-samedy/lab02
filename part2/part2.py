...
def num_eights(x):
    ...
    if x / 10 > 1:
        if x % 10 == 8:
            return 1 + num_eights(x/10 - 0.8)
        return 0 + num_eights((x - x % 10)/10)
    elif x % 10 == 8:
        return 1
    else:
        return 0
...
def pingpong(n):
    ...
    True
    """
    def searcher(index, ppn, dir):
        if n == 1:
            return 1
        elif index != n:
            if num_eights(index) > 0 or index % 8 == 0:
                return searcher(index + 1, ppn - dir, -dir)
            else:
                return searcher(index + 1, ppn + dir, dir)
        else:
            return ppn
    return searcher(1, 1, 1)
...
def count_coins(total):
    ...
    def exchanger(m, n):
        if (m == None):
            ...
            return 0
        with_coin = exchanger(m, n - m)
        without_coin = exchanger(next_largest_coin(m), n)
        return with_coin + without_coin

    return exchanger(1, total)
